...
#define PLAYER_ITEM_USE (GetKeyboardPress(DIK_I) || GetControllerButtonPress(XIP_RB)) //  アイテムを掴んだり離したり

#define ZOOM_MIN (GetKeyboardTrigger(DIK_L) || GetControllerButtonTrigger(XIP_B)) // 最小
#define ZOOM_DEF (GetKeyboardTrigger(DIK_K) || GetControllerButtonTrigger(XIP_Y)) // 普通
#define ZOOM_MAX (GetKeyboardTrigger(DIK_J) || GetControllerButtonTrigger(XIP_X)) // 最大
